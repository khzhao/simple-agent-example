...
from .openai_client import OpenAIChatModel, OpenAIRewardModel
...
async def train(args: argparse.Namespace) -> None:
    ...
    teacher_model = OpenAIChatModel(
        model=args.teacher_openai_model,
        api_key=args.openai_api_key,
        temperature=args.teacher_temperature,
        max_output_tokens=args.teacher_max_tokens,
    )
    reward_model = OpenAIRewardModel(
        model=args.reward_openai_model,
        api_key=args.openai_api_key,
        temperature=args.reward_temperature,
        max_output_tokens=args.reward_max_tokens,
    )
        ...
        "steps": [
            {
                ...
                "teacher_response_text": step.teacher_response_text,
                "raw_reward": step.raw_reward,
                "scaled_reward": step.scaled_reward,
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--teacher-openai-model",
        default=os.getenv("TEACHER_OPENAI_MODEL", "gpt-4o-mini"),
        help="OpenAI model name used for teacher rollouts.",
    )
    parser.add_argument(
        "--teacher-temperature",
        type=float,
        default=0.2,
        help="Sampling temperature for the OpenAI teacher model.",
    )
    parser.add_argument(
        "--teacher-max-tokens",
        type=int,
        default=128,
        help="Maximum tokens generated by the teacher model per move.",
    )
    parser.add_argument(
        "--reward-openai-model",
        default=os.getenv("REWARD_OPENAI_MODEL", "gpt-4o-mini"),
        help="OpenAI model name used for reward scoring.",
    )
    parser.add_argument(
        "--reward-temperature",
        type=float,
        default=0.0,
        help="Sampling temperature for the reward model.",
    )
    parser.add_argument(
        "--reward-max-tokens",
        type=int,
        default=200,
        help="Maximum tokens generated by the reward model.",
    )
